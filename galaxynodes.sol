...
contract NODERewardManagement {
    ...
    uint256 public maxNumOfClaims = 30;
    ...
    function _calculateReward(NodeEntity memory node)
        ...
        returns (uint256)
    {
        if(node.claimCount >= maxNumOfClaims){
            return 0;
        }
    ...
    function updateMaxNumOfClaims(uint256 total) external onlySentry {
        maxNumOfClaims = total;
    }
...
contract Galaxy is ERC20, Ownable, PaymentSplitter {
    ...
    function getNodesClaimCounts() public view returns (string memory) {
        require(_msgSender() != address(0), "SENDER CAN'T BE ZERO");
        require(nodeRewardManager._isNodeOwner(_msgSender()), "NO NODE OWNER");
        return nodeRewardManager._getNodesClaimCounts(_msgSender());
    }
